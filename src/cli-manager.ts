...
export class TaskmasterCLIManager {
  ...
  async execute(options: CLIOptions): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (!child.killed) {
          child.kill();
          reject(new Error('Taskmaster CLI execution timed out after 2 minutes'));
        }
      }, 120000);
    });
  }
  ...
  parseOutput(output: string): any {
    if (!output || output.trim() === '') {
      throw new Error('CLI output is empty');
    }
    ...
    try {
      ...
      for (const task of jsonOutput.tasks) {
        ...
        if (typeof task.id !== 'number') {
          throw new Error(`Invalid task ID: ${task.id} must be a number`);
        }
        
        // Validate dependencies are numbers if present
        if (task.dependencies && Array.isArray(task.dependencies)) {
          for (const dep of task.dependencies) {
            if (typeof dep !== 'number') {
              throw new Error(`Invalid dependency ID: ${dep} must be a number`);
            }
          }
        }
      }
  ...
  async generateTaskGraph(prdFiles: string[], options: CLIOptions): Promise<any> {
    if (!prdFiles || prdFiles.length === 0) {
      throw new Error('No PRD files provided');
    }

    // Validate options
    if (options.complexityThreshold < 0) {
      throw new Error('Complexity threshold must be non-negative');
    }
    ...
    const filteredTasks = parsedOutput.tasks.filter((task: any) => {
    ...
    const taskGraph = {
      master: {
        ...
        metadata: {
          ...
          prdFiles: prdFiles,
          tasksTotal: parsedOutput.tasks.length,
